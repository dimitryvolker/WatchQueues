...
class DrawableContainer{
    ...
    */
    public function addChildContainer(height as Float){
        ...
        if(_children.size() == 0){
            var child = new DrawableContainer(self.start, new Position(self.end.x, (self.height / 100) * height + self.start.y));
            _children.put(_children.size(), child);
            return child;
        }

        var prevChild = _children.get(_children.size() - 1);
        var prevChildEnd = prevChild != null ? prevChild.end : new Position(0,0);
        var child = new DrawableContainer(new Position(self.start.x, prevChildEnd.y), new Position(self.end.x, (self.height / 100) * height + prevChildEnd.y));
        _children.put(_children.size(), child);
        return child;
    }
